...
class ASL:
    ...
    def copy_info(self, index, landmark_info):
        ...
        if landmark_names[index] + "_X" not in self.data and landmark_names[index] + "_Y" not in self.data and landmark_names[index] + "_Z" not in self.data:
            self.data[landmark_names[index] + "_X"] = []
            self.data[landmark_names[index] + "_Y"] = []
            self.data[landmark_names[index] + "_Z"] = []

        self.data[landmark_names[index] + "_X"].append(landmark_info.x)
        self.data[landmark_names[index] + "_Y"].append(landmark_info.y)
        self.data[landmark_names[index] + "_Z"].append(landmark_info.z)
    ...
    def run(self):
        ...
        while self.video.isOpened():
            ...
            if key_pressed == ord('q'):
                ...
                data_frame.to_csv('data/landmark_locations_2.csv', index=False, mode='a')
                break
